...
class SortieConteneurController extends Controller
{
    ...
    /**
     * Archiver une sortie d'ordre (créer une entrée dans prime_archives)
     */
    public function archiverSortie(SortieConteneur $sortie): JsonResponse
    {
        try {
            DB::beginTransaction();

            // Vérifier que la sortie a les champs obligatoires
            if (!$sortie->pv_sortie || !$sortie->pv_rentree_port || !$sortie->numero_ordre) {
                return $this->errorResponse('Les champs PV Sortie, PV Rentrée Port et Numéro d\'Ordre sont obligatoires', 400);
            }

            // Créer l'archive dans prime_archives
            \DB::table('prime_archives')->insert([
                ...
                'date_paiement' => now(),
                'numero_semaine' => now()->format('Y') . '-S' . now()->weekOfYear,
                'paye_par' => auth()->id(),
                'created_at' => now(),
                'updated_at' => now(),
            ]);
            ...
            DB::rollback();
            \Log::error('Erreur archivage sortie: ' . $e->getMessage(), [
                'sortie_id' => $sortie->id,
                'trace' => $e->getTraceAsString()
            ]);
